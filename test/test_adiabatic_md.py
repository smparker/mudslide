...
def test_zero_temperature_dynamics():
    ...
    traj = mudslide.AdiabaticMD(water_model, x, p, dt=20, max_steps=10)
    results = traj.simulate()

    assert np.allclose(x, results[0]["position"])
    assert np.allclose(p, results[0]["momentum"])
    ...
    assert np.allclose(p, results[9]["momentum"])
...
def test_1000K_from_equilibrium():
    ...
    p9 = np.array([-5.06144019,  0.43268942,  8.0867855 , -5.16979919,  1.46354279,        1.17949342, -2.84217795,  1.60202598, -2.15221203])
    V9 = 0.0003035421477246298
    KE9 = 0.013957193155584462

    assert np.allclose(x9, results[9]["position"])
    assert np.allclose(p9, results[9]["momentum"])
