...
class SpawnStack:
    ...
    def next_zeta(
        ...
        izeta = self.izeta
        while (izeta < len(self.sample_stack) and
               self.sample_stack[izeta]["zeta"] < current_value):
            izeta += 1
    ...
    def from_quadrature(
        ...
        random_state : np.random.RandomState, optional
            Random state for generating random numbers, by default None
...
class EvenSamplingTrajectory(SurfaceHoppingMD):
    ...
    **options : dict
    ...
    def hopper(self, gkndt: np.ndarray) -> List[Dict[str, Union[int, float]]]:
        ...
        gkndt[self.state] = 0.0  # ensure self-hopping is nonsense
        gkdt = np.sum(gkndt)
